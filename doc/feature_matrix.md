...
| Range Area charts | `RangeAreaDataPoint`, `RangeAreaChartSeries`, `RangeAreaBoundaryStyle`, `RangeAreaLabelConfig`, `RangeAreaInteractionDetails`, typed low/high tracking, explicit gaps, nested-band and Line composition, atomic motion, grammar authoring through `RangeAreaMark` / `BravenChart.geomRangeArea(low:, high:)` with nullable bounds for gaps, artifacts, Data, and Source | Range Area Charts, Selection — Range Area |
...
| Heatmap charts | `HeatmapDataPoint`, explicit missing cells, optional unequal axis-aligned rectangles through `HeatmapCellBounds`, configurable finite empty-value presentation through `HeatmapEmptyValueStyle`, one or more independently scaled `HeatmapChartSeries` values, sequential/diverging/threshold `HeatmapColorScale`, compact `HeatmapColorLegendGroup` presentation for multiple colour axes, `HeatmapSharedColorDomain` for comparable independent small multiples, portable dim/hide windows through `HeatmapValueFilter`, optional interactive continuous legend filtering, categorical X/Y axes, deterministic entrance and stable-identity update motion, regular-grid and explicit-rectangle viewport culling, host-owned viewport-backed matrices through `HeatmapTileSource`, `HeatmapMatrixDomain`, and `HeatmapViewportController`, portable allowlisted provider reconnection through `HeatmapViewportProviderDescriptor` and `ChartRuntimeBindings`, bounded decoded image presentation through `HeatmapRasterTileSource`, `HeatmapRasterViewportController`, and `HeatmapRasterImageResource`, bounded canonical raster companions through `HeatmapRasterSemanticDescriptor`, portable raster reconnection and named fallbacks through `HeatmapRasterViewportProviderDescriptor`, typed interaction, rendered-cell rectangle selection with touched-cell/source-row/source-column `HeatmapSelectionExpansion`, bounded semantics, typed 2D histogram/density/contour preparation, deterministic matrix clustering, portable dendrogram branch and node geometry, standalone themeable `HeatmapDendrogram` and `HeatmapDendrogramStyle` composition with cached branch paths, configurable automatic screen-space level of detail, bounded labels, and independently shaped, filled, and bordered leaf/merge markers, artifacts, lossless tables, generated Source, Grammar, and fluent configuration | Heatmap Charts, Chart Types, Gallery |
...
| Chart grammar and fluent surface | sealed `Mark` hierarchy (`LineMark`, `AreaMark`, `BarMark`, `ScatterMark`, `CandlestickMark`, `RangeAreaMark`, `TrendMark`), typed `Channel`/`CategoryChannel` encodings, `PlotSpec`, `PlotSpecLowering.lower()`, the `BravenPlot` widget, the chained `BravenChart` facade, coded `GrammarDiagnosticCode` failures, and the opt-in `package:braven_charts/braven_charts_fluent.dart` barrel of generated `withX`/`withoutX`/`inheritX`/`clearX`/`updateX` modifiers | Chart Grammar |
...
partition-radial series. Heatmap accepts exactly one
`HeatmapChartSeries` and cannot mix with another Cartesian family in V1.
