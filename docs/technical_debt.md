...
| **TD-004 — Async Performance Test Timing Precision** | **Obsolete as written; successor cleanup completed.** | Every named `PerformanceMonitor` test path and the production type are absent. BC-0021 removed the unused `test/performance/performance_test_utils.dart`, which contained delay-based frame collection and synthetic memory values but had no maintained callers. |
...
## Active successor item

This rebaseline preserved one current, reproducible concern:
...
Its priority, owner, status, acceptance criteria, and evidence live only in
the shared register. If the item moves or changes scope, update the register
