...
void main() async {
  ...
  final schema = CsvSchema(xColumn: 'timestamp', xType: DataType.timestamp, columns: [
    ColumnDef(name: 'power', type: DataType.float32), // Cyclic power
    ColumnDef(name: 'heart_rate', type: DataType.float32), // Int but float is safe
  ...
  final powerSeries = Series.fromColumns(
      id: 'power_watts', x: rawTable['timestamp'], y: rawTable['power'], xDomain: AxisDomain.time(), yDomain: AxisDomain.numeric(unit: 'W'));
  ...
  final avgPower30s =
      SeriesPipeline(powerSeries).window(WindowSpec.sliding(duration: const Duration(seconds: 30))).reduce({ReducerType.mean}).toRenderSeries();

  // Scenario B: Max Power (Peak) per Minute
  // Find the strongest sprint in each minute block.
  final maxPower1m =
      SeriesPipeline(powerSeries).window(WindowSpec.fixed(duration: const Duration(minutes: 1))).reduce({ReducerType.max}).toRenderSeries();
  ...
  final hrTrend = SeriesPipeline(hrSeries)
      ...
      .reduce({ReducerType.mean, ReducerType.min, ReducerType.max}).toRenderSeries();
...
class CsvSchema {
  ...
  CsvSchema({required this.xColumn, required this.xType, required this.columns});
}
...
class MockCsvLoader {
  static Future<Map<String, dynamic>> load(String path, CsvSchema schema) async {
    return {
...
class Series {
  static void fromColumns({required String id, required dynamic x, required dynamic y, required AxisDomain xDomain, required AxisDomain yDomain}) {}
}
