...
```dart
enum SurfaceParamKind { value, enumType, nestedConfig, triState, listValue, mapValue, excludedFunction, excludedController, excludedByAnnotation }

class SurfaceParam {
  const SurfaceParam({required this.name, required this.dartType, required this.kind,
    required this.isRequired, required this.isNullable, this.defaultCode,
  ...
  final bool isRequired; final bool isNullable; final String? defaultCode;
...
class SurfaceClass {
  const SurfaceClass({required this.name, required this.libraryUri, required this.isConstConstructible,
    required this.hasCopyWith, required this.params, this.sealedVariants = const <String>[],
  ...
  final List<SurfaceParam> params; final List<String> sealedVariants; final List<String> presetFactories;
...
abstract interface class SurfaceEmitter {
  ...
  String emit(SurfaceClass cls, SurfaceModel model); // returns formatted Dart source
}
```

Emission rules (each snapshot-tested): extension named `<ClassName>Fluent` on the class; per non-excluded param `ClassName withParamName(Type value) => copyWith(paramName: value);`; nullable-with-clearFlag → additionally `ClassName clearParamName() => copyWith(clearParamName: true);` (flag name from metadata); triState → `withX(Payload value) => copyWith(x: ChartStyleValue.value(value))`, `clearX() => copyWith(x: const ChartStyleValue<Payload>.none())`, `inheritX() => copyWith(x: const ChartStyleValue<Payload>.inherit())`; combinedSetters replace their individual params entirely; sealed variant helpers only when metadata lists them; every generated file starts with the standard header `// GENERATED by surface_gen — do not edit.` + `// ignore_for_file: ...` per repo lint needs.
...
**Files:** Modify `lib/src/models/chart_series.dart`, `x_axis_config.dart`, `y_axis_config.dart`, `category_axis_config.dart`, `interaction_config.dart`, `cartesian_value_summary_config.dart`, scatter/candlestick/bar model files; regenerate `lib/src/fluent/generated/*`; extend `lib/braven_charts_fluent.dart`.
**Metadata obligations:** XAxisConfig/YAxisConfig `CombinedSetter('withVisibleRange', ['min','max'])` (+ the other assert pairs found in Task 2's audit); YAxisConfig annotated on its const `_internal` pathway — reader must accept a named private const ctor via metadata `presetFactories`/reader fallback (design decided in Task 2 fixtures); ScatterChartSeries clearFlags map for its 6 `clear*` booleans; CartesianValueSummaryConfig excluded: ['onPlacementChanged','controller'], sealedVariants for Presentation/Content.
