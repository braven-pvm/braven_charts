...
/// One representative area chart whose TOP EDGE is driven by a colour channel:
/// a green -> red gradient along the edge (leading-point rule) over the fill,
/// plus a colour-ramp legend. The fill itself is not value-driven (deferred).
library;
...
void main() {
  ...
  setUp(() {
    ...
    goldenFileComparator = _TolerantGoldenFileComparator(
      local.basedir.resolve('grammar_channels_area_golden_test.dart'),
  ...
  testWidgets('area top edge driven by a colour channel: green -> red', (
    tester,
  ) async {
      ...
      MaterialApp(
        ...
        home: Scaffold(
          body: RepaintBoundary(
            key: const ValueKey('grammar-channels-area-golden'),
    ...
    await expectLater(
      find.byKey(const ValueKey('grammar-channels-area-golden')),
      matchesGoldenFile('goldens/area_color.png'),
    );
  });
}
