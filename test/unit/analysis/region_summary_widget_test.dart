...
void main() {
  ...
  group('BravenChartPlusState.computeRegionSummaries', () {
    ...
    testWidgets('computeRegionSummaries() returns summaries for a selected region '
        'with correct per-series metrics via GlobalKey access', (WidgetTester tester) async {
    ...
    testWidgets('computeRegionSummaries() returns empty list when no region is selected', (WidgetTester tester) async {
  ...
  group('BravenChartPlusState overlay (showRegionSummary)', () {
    List<ChartSeries> buildTestSeries() {
      return [
        const LineChartSeries(
          id: 'power',
          points: [ChartDataPoint(x: 1.0, y: 100.0), ChartDataPoint(x: 2.0, y: 200.0), ChartDataPoint(x: 3.0, y: 300.0)],
          color: Colors.blue,
        ),
      ];
    }

    testWidgets('showRegionSummaryOverlay() sets overlay state without throwing', (WidgetTester tester) async {
      // Arrange
      final globalKey = GlobalKey<BravenChartPlusState>();
      ...
      final state = globalKey.currentState!;
      final region = DataRegion(id: 'overlay-test', startX: 1.5, endX: 2.5, source: DataRegionSource.boxSelect, seriesData: const {});
    ...
    testWidgets('hideRegionSummaryOverlay() clears overlay state without throwing', (WidgetTester tester) async {
      // Arrange
      final globalKey = GlobalKey<BravenChartPlusState>();
        ...
        MaterialApp(
          home: Scaffold(
            body: SizedBox(
              width: 800,
              height: 400,
              child: BravenChartPlus(key: globalKey, series: buildTestSeries(), showRegionSummary: true),
            ),
          ),
        ),
      );
      ...
      final region = DataRegion(id: 'hide-test', startX: 1.0, endX: 3.0, source: DataRegionSource.boxSelect, seriesData: const {});
    ...
    testWidgets('showRegionSummaryOverlay() is no-op when showRegionSummary is false', (WidgetTester tester) async {
      ...
      final state = globalKey.currentState!;
      final region = DataRegion(id: 'no-op-test', startX: 1.0, endX: 2.0, source: DataRegionSource.boxSelect, seriesData: const {});
    ...
    testWidgets('didUpdateWidget hides overlay when showRegionSummary toggled to false', (WidgetTester tester) async {
              ...
              body: Column(
                children: [
                  SizedBox(
                    width: 800,
                    height: 300,
                    child: BravenChartPlus(key: globalKey, series: buildTestSeries(), showRegionSummary: showSummary),
                  ),
                  ElevatedButton(onPressed: () => setState(() => showSummary = false), child: const Text('Toggle')),
                ],
              ),
            ),
          ),
      ...
      // Show overlay
      final state = globalKey.currentState!;
      final region = DataRegion(id: 'toggle-test', startX: 1.0, endX: 2.0, source: DataRegionSource.boxSelect, seriesData: const {});
      state.showRegionSummaryOverlay(region);
      await tester.pump();
