...
void main() {
  ...
  group('RegionAnalyzer.filterPointsInRange', () {
    ...
    group('sorted data (binary search path, isSorted: true)', () {
      test('returns points within range for sorted ascending data', () {
        ...
        final result = analyzer.filterPointsInRange(points, startX: 3.0, endX: 7.0);

        // Assert
        expect(result, hasLength(5));
        expect(result.map((p) => p.x), containsAllInOrder([3.0, 4.0, 5.0, 6.0, 7.0]));
      });
      ...
      test('returns correct Y values for filtered range', () {
        ...
        final result = analyzer.filterPointsInRange(points, startX: 2.0, endX: 4.0, isSorted: true);
    ...
    group('unsorted data (linear scan fallback, isSorted: false)', () {
      test('returns points within range for unsorted data', () {
        ...
        final result = analyzer.filterPointsInRange(points, startX: 3.0, endX: 7.0, isSorted: false);

        // Assert — should contain same 5 points regardless of order
        expect(result, hasLength(5));
        expect(result.map((p) => p.x).toSet(), equals({3.0, 4.0, 5.0, 6.0, 7.0}));
      });
      ...
      test('returns same results as sorted path for equivalent data', () {
        ...
        // Act
        final result = analyzer.filterPointsInRange(unsortedPoints, startX: 3.0, endX: 7.0, isSorted: false);
    ...
    group('inclusive boundaries', () {
      test('includes points exactly at startX boundary', () {
        // Arrange
        final points = [const ChartDataPoint(x: 1.0, y: 10.0), const ChartDataPoint(x: 2.0, y: 20.0), const ChartDataPoint(x: 3.0, y: 30.0)];

        // Act
        final result = analyzer.filterPointsInRange(points, startX: 2.0, endX: 5.0);
      ...
      test('includes points exactly at endX boundary', () {
        ...
        // Act
        final result = analyzer.filterPointsInRange(points, startX: 0.0, endX: 2.0);

        // Assert
        expect(result.any((p) => p.x == 2.0), isTrue);
      });

      test('includes points at both startX and endX boundaries simultaneously', () {
        // Arrange
        final points = [
          const ChartDataPoint(x: 1.0, y: 10.0),
          const ChartDataPoint(x: 2.0, y: 20.0),
        ...
        // Act
        final result = analyzer.filterPointsInRange(points, startX: 2.0, endX: 4.0);
    ...
    group('empty input', () {
      test('returns empty list when input points list is empty', () {
        ...
        // Act
        final result = analyzer.filterPointsInRange(points, startX: 0.0, endX: 10.0);
    ...
    group('boundary duplicates', () {
      test('includes all duplicate points at the startX boundary', () {
        ...
        // Act
        final result = analyzer.filterPointsInRange(points, startX: 3.0, endX: 5.0);
    ...
    group('single-point match', () {
      test('returns matching point when startX equals endX', () {
        ...
        // Act
        final result = analyzer.filterPointsInRange(points, startX: 3.0, endX: 3.0);
      ...
      test('returns empty when no point matches single-point query', () {
        // Arrange
        final points = [const ChartDataPoint(x: 1.0, y: 10.0), const ChartDataPoint(x: 3.0, y: 30.0), const ChartDataPoint(x: 5.0, y: 50.0)];

        // Act — query at x=2.0 where no point exists
        final result = analyzer.filterPointsInRange(points, startX: 2.0, endX: 2.0);
    ...
    group('range outside data', () {
      test('returns empty when range is entirely before all data points', () {
        // Arrange
        final points = [const ChartDataPoint(x: 5.0, y: 50.0), const ChartDataPoint(x: 6.0, y: 60.0), const ChartDataPoint(x: 7.0, y: 70.0)];

        // Act
        final result = analyzer.filterPointsInRange(points, startX: 1.0, endX: 4.0);
      ...
      test('returns empty when range is entirely after all data points', () {
        ...
        // Act
        final result = analyzer.filterPointsInRange(points, startX: 10.0, endX: 20.0);

        // Assert
        expect(result, isEmpty);
      });

      test('returns empty when range falls between non-contiguous data points', () {
        ...
        final result = analyzer.filterPointsInRange(points, startX: 4.0, endX: 6.0);
  ...
  group('RegionAnalyzer.regionFromAnnotation', () {
    group('builds DataRegion from RangeAnnotation and multi-series data', () {
      test('creates DataRegion with correct source and X-range', () {
        // Arrange
        final annotation = RangeAnnotation(id: 'test-range', startX: 2.0, endX: 8.0, label: 'Test Range');
      ...
      test('maps filtered points correctly per series', () {
        // Arrange
        final annotation = RangeAnnotation(id: 'multi-series-range', startX: 3.0, endX: 7.0);
        final allSeriesData = <String, List<ChartDataPoint>>{
          'power': [
            const ChartDataPoint(x: 1.0, y: 100.0),
            const ChartDataPoint(x: 3.0, y: 300.0),
        ...
        expect(region.seriesData['power']!.map((p) => p.x).toList(), equals([3.0, 5.0, 7.0]));

        // Assert — heartrate series: x=4,6 (2 points in range)
        expect(region.seriesData['heartrate'], hasLength(2));
        expect(region.seriesData['heartrate']!.map((p) => p.x).toList(), equals([4.0, 6.0]));
      });
    });
    ...
    group('excludes series with no matching points', () {
      test('omits series with zero points in range from seriesData', () {
        // Arrange
        final annotation = RangeAnnotation(id: 'partial-range', startX: 3.0, endX: 5.0);
        final allSeriesData = <String, List<ChartDataPoint>>{
          'in-range': [const ChartDataPoint(x: 3.0, y: 30.0), const ChartDataPoint(x: 4.0, y: 40.0)],
          'out-of-range': [const ChartDataPoint(x: 1.0, y: 10.0), const ChartDataPoint(x: 2.0, y: 20.0), const ChartDataPoint(x: 6.0, y: 60.0)],
        };
    ...
    group('handles empty series gracefully', () {
      test('returns DataRegion with empty seriesData when no series provided', () {
        // Arrange
        final annotation = RangeAnnotation(id: 'empty-range', startX: 3.0, endX: 5.0);
        final allSeriesData = <String, List<ChartDataPoint>>{};
      ...
      test('returns DataRegion with empty seriesData when series have empty point lists', () {
        // Arrange
        final annotation = RangeAnnotation(id: 'empty-points-range', startX: 1.0, endX: 10.0);
        final allSeriesData = <String, List<ChartDataPoint>>{'empty-series': <ChartDataPoint>[]};
      ...
      test('handles mix of populated and empty series', () {
        // Arrange
        final annotation = RangeAnnotation(id: 'mix-range', startX: 2.0, endX: 6.0);
        final allSeriesData = <String, List<ChartDataPoint>>{
          'has-data': [const ChartDataPoint(x: 3.0, y: 30.0), const ChartDataPoint(x: 5.0, y: 50.0)],
  ...
  group('RegionAnalyzer.computeSeriesSummary', () {
    ...
    group('50-point series with all metrics', () {
      ...
      final points = List.generate(50, (i) => ChartDataPoint(x: i.toDouble(), y: i * 2.0 + 1.0));

      test('returns non-null SeriesRegionSummary for 50-point input', () {
        final result = analyzer.computeSeriesSummary(points, seriesId: 'test-series', regionStartX: 0.0, regionEndX: 49.0);
      ...
      test('duration equals regionEndX - regionStartX (49.0) per data-model spec', () {
      ...
      test('seriesId is passed through correctly', () {
        final result = analyzer.computeSeriesSummary(points, seriesId: 'my-series-id', regionStartX: 0.0, regionEndX: 49.0);
    ...
    group('1-point series', () {
      ...
      test('returns non-null for 1-point input', () {
        final result = analyzer.computeSeriesSummary(singlePoint, seriesId: 'single', regionStartX: 0.0, regionEndX: 10.0);
        expect(result, isNotNull);
      });
    ...
    group('0-point (empty) series', () {
      test('returns null for empty points list', () {
        final result = analyzer.computeSeriesSummary(<ChartDataPoint>[], seriesId: 'empty', regionStartX: 0.0, regionEndX: 10.0);
    ...
    group('floating-point precision', () {
      test('handles values requiring closeTo precision for fractional data', () {
        // Points with fractional values that could cause precision issues
        final points = [const ChartDataPoint(x: 0.0, y: 0.1), const ChartDataPoint(x: 1.0, y: 0.2), const ChartDataPoint(x: 2.0, y: 0.3)];

        final result = analyzer.computeSeriesSummary(points, seriesId: 'precision', regionStartX: 0.0, regionEndX: 2.0);
  ...
  group('RegionAnalyzer.computeRegionSummary', () {
    ...
    group('multi-series DataRegion', () {
      test('returns RegionSummary with per-series SeriesRegionSummary entries', () {
        ...
        final region = DataRegion(
          ...
          seriesData: const {
            'power': [ChartDataPoint(x: 1.0, y: 100.0), ChartDataPoint(x: 3.0, y: 300.0), ChartDataPoint(x: 5.0, y: 200.0)],
            'heartrate': [ChartDataPoint(x: 2.0, y: 70.0), ChartDataPoint(x: 4.0, y: 120.0), ChartDataPoint(x: 6.0, y: 90.0)],
          },
        );
      ...
      test('each series summary has correct metric values', () {
        ...
        final region = DataRegion(
          ...
          source: DataRegionSource.rangeAnnotation,
          seriesData: const {
            'temperature': [ChartDataPoint(x: 1.0, y: 20.0), ChartDataPoint(x: 2.0, y: 30.0), ChartDataPoint(x: 3.0, y: 25.0)],
          },
        );
    ...
    group('empty series omission', () {
      test('omits series with zero points from seriesSummaries map', () {
        ...
        final region = DataRegion(
          ...
          source: DataRegionSource.rangeAnnotation,
          seriesData: const {
            'has-data': [ChartDataPoint(x: 1.0, y: 10.0), ChartDataPoint(x: 2.0, y: 20.0)],
            'empty': <ChartDataPoint>[],
          },
        );
      ...
      test('returns RegionSummary with empty map when all series are empty', () {
        // Arrange
        final region = DataRegion(
          id: 'all-empty',
          startX: 0.0,
          endX: 10.0,
          source: DataRegionSource.rangeAnnotation,
          seriesData: const {'empty-a': <ChartDataPoint>[], 'empty-b': <ChartDataPoint>[]},
        );
    ...
    group('seriesNames and seriesUnits pass-through', () {
      test('custom seriesNames are passed to SeriesRegionSummary objects', () {
        ...
        final region = DataRegion(
          ...
          source: DataRegionSource.rangeAnnotation,
          seriesData: const {
            'power': [ChartDataPoint(x: 1.0, y: 100.0), ChartDataPoint(x: 2.0, y: 200.0)],
            'heartrate': [ChartDataPoint(x: 1.0, y: 70.0), ChartDataPoint(x: 2.0, y: 90.0)],
          },
        );

        // Act
        final result = analyzer.computeRegionSummary(region, seriesNames: {'power': 'Power Output', 'heartrate': 'Heart Rate'});

        // Assert
        expect(result.seriesSummaries['power']!.seriesName, equals('Power Output'));
        expect(result.seriesSummaries['heartrate']!.seriesName, equals('Heart Rate'));
      });
      ...
      test('custom seriesUnits are passed to SeriesRegionSummary objects', () {
        ...
        final region = DataRegion(
          ...
          seriesData: const {
            'power': [ChartDataPoint(x: 1.0, y: 100.0), ChartDataPoint(x: 2.0, y: 200.0)],
            'temperature': [ChartDataPoint(x: 1.0, y: 20.0), ChartDataPoint(x: 2.0, y: 25.0)],
          },
        );

        // Act
        final result = analyzer.computeRegionSummary(region, seriesUnits: {'power': 'W', 'temperature': '°C'});
      ...
      test('seriesNames and seriesUnits work together', () {
        ...
        final region = DataRegion(
          ...
          source: DataRegionSource.rangeAnnotation,
          seriesData: const {
            'speed': [ChartDataPoint(x: 1.0, y: 30.0), ChartDataPoint(x: 2.0, y: 35.0)],
          },
        );

        // Act
        final result = analyzer.computeRegionSummary(region, seriesNames: {'speed': 'Speed'}, seriesUnits: {'speed': 'km/h'});
      ...
      test('series without name/unit mappings have null seriesName and unit', () {
        // Arrange
        final region = DataRegion(
          id: 'partial-metadata',
          startX: 0.0,
        ...
        // Act
        final result = analyzer.computeRegionSummary(region, seriesNames: {'mapped': 'Mapped Series'}, seriesUnits: {'mapped': 'units'});

        // Assert — 'mapped' has metadata, 'unmapped' does not
        expect(result.seriesSummaries['mapped']!.seriesName, equals('Mapped Series'));
  ...
  group('RegionAnalyzer.detectSegmentGroups', () {
    ...
    group('contiguous same-style groups', () {
      test('detects a single contiguous group of same-style points', () {
        ...
        final points = List.generate(5, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle));
      ...
      test('detects two contiguous groups with different styles', () {
        ...
        final points = <ChartDataPoint>[
          ...List.generate(5, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle)),
          ...List.generate(5, (i) => ChartDataPoint(x: (i + 5).toDouble(), y: (i + 5) * 10.0, segmentStyle: redStyle)),
        ];
      ...
      test('groups three contiguous segments with alternating styles', () {
        ...
        final points = <ChartDataPoint>[
          ...List.generate(3, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle)),
          ...List.generate(3, (i) => ChartDataPoint(x: (i + 3).toDouble(), y: (i + 3) * 10.0, segmentStyle: redStyle)),
          ...List.generate(3, (i) => ChartDataPoint(x: (i + 6).toDouble(), y: (i + 6) * 10.0, segmentStyle: greenStyle)),
        ];
    ...
    group('non-adjacent same-style groups (FR-017 contiguity)', () {
      test('non-adjacent same-style groups are separate DataRegions', () {
        ...
        final points = <ChartDataPoint>[
          ...List.generate(5, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle)),
          ...List.generate(5, (i) => ChartDataPoint(x: (i + 5).toDouble(), y: (i + 5) * 10.0, segmentStyle: redStyle)),
          ...List.generate(5, (i) => ChartDataPoint(x: (i + 10).toDouble(), y: (i + 10) * 10.0, segmentStyle: blueStyle)),
        ];
      ...
      test('non-adjacent same-style with unstyled gap produces separate groups', () {
        ...
        final points = <ChartDataPoint>[
          ...
          const ChartDataPoint(x: 3.0, y: 30.0),
          const ChartDataPoint(x: 4.0, y: 40.0),
          ...List.generate(3, (i) => ChartDataPoint(x: (i + 5).toDouble(), y: (i + 5) * 10.0, segmentStyle: blueStyle)),
        ];
    ...
    group('unstyled points excluded', () {
      test('returns empty list when all points are unstyled', () {
        ...
        final points = List.generate(10, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0));
      ...
      test('unstyled points between styled groups are excluded', () {
        ...
        final points = <ChartDataPoint>[
          ...List.generate(3, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle)),
          // Unstyled gap
          ...List.generate(4, (i) => ChartDataPoint(x: (i + 3).toDouble(), y: (i + 3) * 10.0)),
          ...List.generate(3, (i) => ChartDataPoint(x: (i + 7).toDouble(), y: (i + 7) * 10.0, segmentStyle: blueStyle)),
        ];
    ...
    group('all-same-style series', () {
      test('all points with same style form a single group', () {
        // Arrange
        const blueStyle = SegmentStyle(color: Color(0xFF0000FF));
        final points = List.generate(20, (i) => ChartDataPoint(x: i.toDouble(), y: i * 5.0, segmentStyle: blueStyle));
    ...
    group('segment ID format and source type', () {
      test('segment group ID follows format segment_<seriesId>_<startIndex>', () {
        // Arrange
        const blueStyle = SegmentStyle(color: Color(0xFF0000FF));
        const redStyle = SegmentStyle(color: Color(0xFFFF0000));
      ...
      test('segment highlight range excludes adjacent group', () {
        // Arrange — two adjacent styled groups with a trailing point
        const blueStyle = SegmentStyle(color: Color(0xFF0000FF));
        const redStyle = SegmentStyle(color: Color(0xFFFF0000));
        ...
        // Compute highlight for 'next' group the same way BravenChartPlus does
        // Find the last *styled* point in the next group (seriesData may include a trailing endpoint).
        final lastStyled = next.seriesData['series-a']!.where((p) => p.segmentStyle != null).last;
        final lastIdx = points.indexWhere((p) => p.x == lastStyled.x && p.y == lastStyled.y);
        final highlightStartX = next.startX;
        final highlightEndX = (lastIdx >= 0 && lastIdx + 1 < points.length) ? points[lastIdx + 1].x : lastStyled.x;

        // Prev group's exclusive end-X is the X of the point after its last styled point
        // Use the group's declared endX to find the last *styled* point —
        // seriesData may include a trailing endpoint which should not be
        // treated as part of the styled count.
        final prevLastIdx = points.indexWhere(
          (p) => p.x == prev.endX && p.y == prev.seriesData['series-a']!.where((pt) => pt.x == prev.endX).first.y,
        );
        final prevEndExclusive = (prevLastIdx >= 0 && prevLastIdx + 1 < points.length) ? points[prevLastIdx + 1].x : prev.endX;

        // Assert — highlight start must be >= prevEndExclusive (no overlap)
        expect(highlightStartX >= prevEndExclusive, isTrue);
      });
    });
    ...
    group('empty input', () {
      test('returns empty list for empty points', () {
        final groups = analyzer.detectSegmentGroups('series-a', <ChartDataPoint>[]);
        expect(groups, isEmpty);
      });
    });
    ...
    group('seriesData content', () {
      test('seriesData contains actual points from contiguous group', () {
        ...
        // First group should contain the blue points plus the trailing endpoint
        expect(groups[0].seriesData.containsKey('series-a'), isTrue);
        expect(groups[0].seriesData['series-a'], hasLength(4));
        expect(groups[0].seriesData['series-a']!.map((p) => p.x).toList(), equals([0.0, 1.0, 2.0, 3.0]));

        // Second group should contain the red points (no trailing point available)
        expect(groups[1].seriesData.containsKey('series-a'), isTrue);
        expect(groups[1].seriesData['series-a'], hasLength(3));
        expect(groups[1].seriesData['series-a']!.map((p) => p.x).toList(), equals([3.0, 4.0, 5.0]));
      });
    });
  });
  ...
  group('RegionAnalyzer.segmentGroupForPoint', () {
    ...
    group('point inside a group', () {
      test('returns correct DataRegion for a point in the middle of a group', () {
        // Arrange — points 0-9 styled blue, 10-19 styled red
        const blueStyle = SegmentStyle(color: Color(0xFF0000FF));
        const redStyle = SegmentStyle(color: Color(0xFFFF0000));

        final points = <ChartDataPoint>[
          ...List.generate(10, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle)),
          ...List.generate(10, (i) => ChartDataPoint(x: (i + 10).toDouble(), y: (i + 10) * 10.0, segmentStyle: redStyle)),
        ];
    ...
    group('point outside any group', () {
      test('returns null for an unstyled point', () {
        ...
        final points = <ChartDataPoint>[
          ...List.generate(5, (i) => ChartDataPoint(x: i.toDouble(), y: i * 10.0, segmentStyle: blueStyle)),
          ...List.generate(5, (i) => ChartDataPoint(x: (i + 5).toDouble(), y: (i + 5) * 10.0)),
        ];
    ...
    group('contiguity verification', () {
      test('non-adjacent same-style: tap point 12 returns only group 10-14', () {
        // Arrange — blue 0-4, red 5-9, blue 10-14
        const blueStyle = SegmentStyle(color: Color(0xFF0000FF));
        const redStyle = SegmentStyle(color: Color(0xFFFF0000));
  ...
  group('Edge-case: region beyond data boundaries', () {
    test('startX before first data point returns only actual points in range', () {
      // Arrange — data starts at x=10; query starts at x=0 (before data)
      final points = [const ChartDataPoint(x: 10.0, y: 100.0), const ChartDataPoint(x: 20.0, y: 200.0), const ChartDataPoint(x: 30.0, y: 300.0)];

      // Act — startX=0.0 is before the first data point (x=10)
      final result = analyzer.filterPointsInRange(points, startX: 0.0, endX: 25.0);
    ...
    test('endX after last data point returns only actual points in range', () {
      ...
      final result = analyzer.filterPointsInRange(points, startX: 15.0, endX: 50.0);
    ...
    test('range spanning the entire data set returns all data points', () {
      ...
      final points = [const ChartDataPoint(x: 5.0, y: 50.0), const ChartDataPoint(x: 10.0, y: 100.0), const ChartDataPoint(x: 15.0, y: 150.0)];

      // Act — query range is much wider than the data
      final result = analyzer.filterPointsInRange(points, startX: -100.0, endX: 100.0);
  ...
  group('Edge-case: unsorted data correctness (isSorted:false)', () {
    test('isSorted:false with random-order points returns same values as '
      ...
      final sortedResult = analyzer.filterPointsInRange(sortedPoints, startX: 2.0, endX: 6.0, isSorted: true);
      final unsortedResult = analyzer.filterPointsInRange(unsortedPoints, startX: 2.0, endX: 6.0, isSorted: false);

      // Assert — same points returned regardless of order
      expect(sortedResult, hasLength(5));
      expect(unsortedResult, hasLength(5));
      expect(sortedResult.map((p) => p.x).toSet(), equals(unsortedResult.map((p) => p.x).toSet()));
      expect(sortedResult.map((p) => p.y).toSet(), equals(unsortedResult.map((p) => p.y).toSet()));
    });

    test('isSorted:false with strictly descending data produces correct results', () {
      ...
      final result = analyzer.filterPointsInRange(descendingPoints, startX: 30.0, endX: 70.0, isSorted: false);
  ...
  group('Edge-case: duplicate X values at boundary (inclusive)', () {
    test('multiple points sharing startX value are ALL included', () {
      ...
      final result = analyzer.filterPointsInRange(points, startX: 5.0, endX: 8.0);
    ...
    test('multiple points sharing endX value are ALL included', () {
      ...
      final result = analyzer.filterPointsInRange(points, startX: 6.0, endX: 10.0);
    ...
    test('duplicates at both startX and endX are ALL included', () {
      ...
      final result = analyzer.filterPointsInRange(points, startX: 4.0, endX: 8.0);
  ...
  group('Edge-case: multi-axis filtering uses raw coordinates (FR-008)', () {
    test('two series on different Y-scales are filtered independently by X', () {
      ...
      final powerFiltered = analyzer.filterPointsInRange(powerSeriesPoints, startX: regionStartX, endX: regionEndX);

      final hrFiltered = analyzer.filterPointsInRange(hrSeriesPoints, startX: regionStartX, endX: regionEndX);

      // Assert — power series: x=20,30,40 (3 points)
      expect(powerFiltered, hasLength(3));
      expect(powerFiltered.map((p) => p.x).toSet(), equals({20.0, 30.0, 40.0}));
      // Raw Y values preserved — normalization does NOT alter the data
      expect(powerFiltered.map((p) => p.y).toSet(), equals({300.0, 380.0, 420.0}));

      // Assert — hr series: x=15,25,35,45 (4 points)
      expect(hrFiltered, hasLength(4));
      expect(hrFiltered.map((p) => p.x).toSet(), equals({15.0, 25.0, 35.0, 45.0}));
      // Raw Y values preserved — heart rate values are in bpm, not normalised
      expect(hrFiltered.map((p) => p.y).toSet(), equals({130.0, 145.0, 160.0, 155.0}));
    });

    test('series with sparse data and different X densities are filtered correctly', () {
      // Arrange — two series with very different data densities
      // (one measured every second, one every 10 seconds)
      final denseSeriesPoints = List.generate(100, (i) => ChartDataPoint(x: i.toDouble(), y: i * 3.0));

      final sparseSeriesPoints = List.generate(10, (i) => ChartDataPoint(x: (i * 10).toDouble(), y: i * 50.0));

      // Act — same region for both
      final denseFiltered = analyzer.filterPointsInRange(denseSeriesPoints, startX: 20.0, endX: 50.0);

      final sparseFiltered = analyzer.filterPointsInRange(sparseSeriesPoints, startX: 20.0, endX: 50.0);
      ...
      expect(sparseFiltered.map((p) => p.x).toSet(), equals({20.0, 30.0, 40.0, 50.0}));
    });
  });
  ...
  group('Edge-case: streaming snapshot analysis', () {
    test('analysis on data snapshot produces correct summary from available data', () {
      ...
      final result = analyzer.computeSeriesSummary(snapshotPoints, seriesId: 'power', regionStartX: 0.0, regionEndX: 30.0);
      ...
      final laterPoints = [const ChartDataPoint(x: 35.0, y: 350.0), const ChartDataPoint(x: 40.0, y: 380.0), const ChartDataPoint(x: 45.0, y: 400.0)];
    ...
    test('partial stream region analysis gives statistically correct results', () {
      // Arrange — 50 points available mid-stream (out of eventual 100)
      final availablePoints = List.generate(50, (i) => ChartDataPoint(x: i.toDouble() * 2.0, y: 100.0 + i * 4.0));
      // x ranges from 0.0 to 98.0, y ranges from 100.0 to 296.0

      // Act — region analysis on currently available data only
      final result = analyzer.computeSeriesSummary(availablePoints, seriesId: 'heartrate', regionStartX: 0.0, regionEndX: 98.0);

      // Assert — correct statistics for the 50 available points
      expect(result, isNotNull);
      expect(result!.count, equals(50));
      expect(result.min, closeTo(100.0, 1e-10)); // y at x=0
      expect(result.max, closeTo(296.0, 1e-10)); // y at x=98 (i=49: 100+49*4=296)
    ...
    test('regionFromAnnotation uses data snapshot — only currently-known '
      ...
      final annotation = RangeAnnotation(id: 'effort-zone', startX: 0.0, endX: 60.0);
      ...
      final region = analyzer.regionFromAnnotation(annotation, currentSeriesData);
