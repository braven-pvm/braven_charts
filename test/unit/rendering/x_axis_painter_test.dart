import 'dart:ui' show Canvas, Color, PictureRecorder, Rect, TextDirection;
  ...
  group('XAxisPainter', () {
    ...
    group('categorical ticks', () {
      XAxisPainter painter({
        ...
        CategoryLabelOverflow overflow = CategoryLabelOverflow.wrap,
      }) => XAxisPainter(
        config: XAxisConfig(
          categoryAxis: CategoryAxisConfig(
            ...
            minimumCategoryExtent: 60,
          ),
        ),
        axisBounds: const DataRange(min: -0.5, max: 5.5),
        labelStyle: const TextStyle(fontSize: 12),
      );

      test('uses category centers and automatically thins dense labels', () {
        expect(painter().resolveTickValues(400), [0, 1, 2, 3, 4, 5]);
        expect(painter().resolveTickValues(180), [0, 2, 4, 5]);
      });

      test('showAll preserves every visible category tick', () {
        expect(
          painter(density: CategoryLabelDensity.showAll).resolveTickValues(80),
          [0, 1, 2, 3, 4, 5],
        );
      });

      test('category labels take precedence over callback formatters', () {
        final categoryPainter = XAxisPainter(
          config: XAxisConfig(
            categoryAxis: const CategoryAxisConfig(
              categories: ['Alpha', 'Beta'],
            ),
            labelFormatter: (value) => 'numeric $value',
          ),
          axisBounds: const DataRange(min: -0.5, max: 1.5),
          labelStyle: const TextStyle(fontSize: 12),
        );

        expect(categoryPainter.formatTickLabel(1), 'Beta');
        expect(categoryPainter.formatTickLabel(0.5), 'numeric 0.5');
      });

      test('wrap and ellipsis policies produce bounded text layouts', () {
        final wrapped = painter().layoutTickLabel(0, maximumWidth: 58);
        final ellipsized = painter(
          overflow: CategoryLabelOverflow.ellipsis,
        ).layoutTickLabel(0, maximumWidth: 58);

        expect(wrapped.height, greaterThan(ellipsized.height));
        expect(ellipsized.didExceedMaxLines, isTrue);
      });

      test('inherits right-to-left direction for category labels', () {
        final rtl = XAxisPainter(
          config: const XAxisConfig(
            categoryAxis: CategoryAxisConfig(categories: ['الإيرادات']),
          ),
          axisBounds: const DataRange(min: -0.5, max: 0.5),
          labelStyle: const TextStyle(fontSize: 12),
          textDirection: TextDirection.rtl,
        );

        expect(rtl.layoutTickLabel(0).textDirection, TextDirection.rtl);
      });
    });
