  ...
  group('Edge Case: Extreme Zoom Out', () {
    test('Viewport shows all 10,000 points (graceful degradation)', () {
      final pipeline = RenderPipeline(
        ...
        initialViewport: Rect.fromLTWH(0, 0, 10000, 100),  // Show all data
      );

      // Create 10,000 data points
      final dataPoints =
          List.generate(10000, (i) => ChartDataPoint(i.toDouble(), (i % 100).toDouble()));

      pipeline.addLayer(DataSeriesLayer(
        dataPoints: dataPoints,
        dataBounds: Rect.fromLTRB(0, 0, 10000, 100),
      ...
      expect(() => pipeline.renderFrame(canvas, const Size(800, 600)), returnsNormally,
          reason: 'Should not crash when rendering all 10,000 points');
      ...
      expect(frameTime, lessThan(50),
          reason: 'Frame time should degrade gracefully (<50ms for 10K points)');
    ...
    test('No crashes with maximum data set', () {
      final pipeline = RenderPipeline(
        ...
        performanceMonitor: StopwatchPerformanceMonitor(),
        culler: const ViewportCuller(),
        initialViewport: Rect.fromLTWH(0, 0, 10000, 100),
      );
      ...
      for (int i = 0; i < 10; i++) {
        expect(() => pipeline.renderFrame(canvas, const Size(800, 600)), returnsNormally,
            reason: 'Frame $i should not crash');
      }
    ...
    test('Frame time comparison: zoomed in vs zoomed out', () {
      final pipeline = RenderPipeline(
        ...
        performanceMonitor: StopwatchPerformanceMonitor(),
        culler: const ViewportCuller(),
        initialViewport: Rect.fromLTWH(0, 0, 800, 100),
      );
      ...
      pipeline.updateViewport(Rect.fromLTWH(0, 0, 800, 100));  // ~800 points visible
      ...
      pipeline.updateViewport(Rect.fromLTWH(0, 0, 10000, 100));  // All 10,000 points
      ...
      expect(slowdown, lessThan(20),
          reason: 'Slowdown should be roughly proportional to visible points');
    ...
    test('Pool statistics under extreme load', () {
      ...
      expect(finalPaintStats.acquireCount, greaterThan(initialPaintStats.acquireCount),
          reason: 'Paint pool should have been used');
      expect(finalPathStats.acquireCount, greaterThan(initialPathStats.acquireCount),
          reason: 'Path pool should have been used');
    ...
    test('Rendering artifacts validation (basic sanity check)', () {
      ...
      expect(pipeline.viewport, equals(Rect.fromLTWH(0, 0, 10000, 100)),
          reason: 'Viewport should remain unchanged');

      expect(pipeline.layers.length, equals(1),
          reason: 'Layer count should remain unchanged');
