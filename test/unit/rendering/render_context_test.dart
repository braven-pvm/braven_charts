...
void main() {
  ...
  RenderContext createValidContext({
    ...
    return RenderContext(
      ...
      viewport: viewport ?? Rect.fromLTWH(0, 0, 800, 600),
  ...
  group('RenderContext - Viewport Handling', () {
    ...
    test('viewport can be smaller than canvas (zoomed in)', () {
      const size = Size(800, 600);
      final viewport = Rect.fromLTWH(100, 100, 400, 300); // Zoomed region
    ...
    test('viewport can extend beyond canvas (panned out)', () {
      const size = Size(800, 600);
      final viewport = Rect.fromLTWH(-200, -100, 1200, 800); // Extends beyond
