...
class ChartRenderBox extends RenderBox {
  ...
  /// Streaming data storage, keyed by series ID.
  ///
  /// This holds data from LiveStreamController, separate from the static
  /// elements generated by widget.series. During paint, streaming data is
  /// rendered as an additional layer.
  final Map<String, List<ChartDataPoint>> _streamingData = {};

  /// Cached streaming series elements, regenerated when data or transform changes.
  ...
  /// - [points]: Complete list of points in the streaming buffer
  /// - [bounds]: Pre-calculated data bounds for efficient viewport updates
  ///
  /// **Performance**: O(n) where n = number of points, but no widget rebuild.
  void setStreamingData({
    ...
    // Store data
    _streamingData[seriesId] = points;
    ...
    // Without this, the scrollbar and pan constraints use the initial series bounds
    // which may be 0-1 from an empty series.
    if (_originalTransform != null && points.isNotEmpty) {
      final yPadding = (yMax - yMin) * 0.05;

      // DEFENSIVE: If existing transform has invalid bounds, recreate it entirely
      // instead of calling copyWith (which would fail assertion)
      if (_originalTransform!.dataXMax <= _originalTransform!.dataXMin || _originalTransform!.dataYMax <= _originalTransform!.dataYMin) {
        _originalTransform = ChartTransform(
          dataXMin: xMin,
          dataXMax: xMax,
          dataYMin: yMin - yPadding,
          dataYMax: yMax + yPadding,
          plotWidth: _originalTransform!.plotWidth,
          plotHeight: _originalTransform!.plotHeight,
        ...
        _originalTransform = _originalTransform!.copyWith(
          dataXMin: xMin, // Start from data X=0
          dataXMax: xMax, // Include latest point
          dataYMin: yMin - yPadding,
          dataYMax: yMax + yPadding,
        );
      }
    ...
    // CRITICAL: Update transform Y bounds to match streaming data
    // When static series has empty points, the initial transform has range 0-1,
    // but streaming data might have completely different range. Without this update,
    // the streaming data would be rendered outside the visible area.
    //
    // NOTE: We do NOT update X bounds here because auto-scroll handles viewport positioning.
    // Updating X bounds here would conflict with auto-scroll's fixed viewport width,
    // causing the viewport to toggle between fixed width and full data range (vibration).
    if (_transform != null && points.isNotEmpty) {
      // Check if we're in initial state (default 0-1 bounds from empty series)
      final isInitialState = _transform!.dataXMin == 0 && _transform!.dataXMax == 1 && _transform!.dataYMin == 0 && _transform!.dataYMax == 1;

      // For initial state, set X bounds once to position viewport
      // After that, auto-scroll takes over X positioning
      final needsInitialXSetup = isInitialState;
      final needsYUpdate = isInitialState || yMin < _transform!.dataYMin || yMax > _transform!.dataYMax;

      if (needsInitialXSetup || needsYUpdate) {
        // Add 5% padding to Y bounds for visual comfort
        final yPadding = (yMax - yMin) * 0.05;

        _transform = _transform!.copyWith(
          dataXMin: needsInitialXSetup ? xMin : _transform!.dataXMin,
          dataXMax: needsInitialXSetup ? xMax : _transform!.dataXMax,
          dataYMin: needsYUpdate ? (yMin - yPadding) : _transform!.dataYMin,
          dataYMax: needsYUpdate ? (yMax + yPadding) : _transform!.dataYMax,
        );
        _updateAxesFromTransform();
      }
    }

    // Regenerate streaming element for this series
    _regenerateStreamingElement(seriesId);

    // PERFORMANCE: Do NOT invalidate series cache for streaming updates!
    // Streaming data is painted separately (not cached) to avoid Picture regeneration
    // at 60fps. Cache remains valid for static series and only invalidates on zoom/pan.
    // _seriesCacheDirty = true;  // REMOVED - causes cache thrashing
  ...
  void clearStreamingData(String seriesId) {
    _streamingData.remove(seriesId);
  ...
  void lockViewportForPause() {
    ...
    // CRITICAL: Use streaming bounds directly, not _originalTransform,
    // because _originalTransform might be from empty initial series (0-1 bounds)
    if (_streamingBounds != null) {
      // Add padding for comfortable panning
      final yPadding = (_streamingBounds!.yMax - _streamingBounds!.yMin) * 0.05;

      setPanConstraintBounds(
  ...
  void snapViewportToStreamingData({
    ...
    if (_viewportLockedForPause) return; // Don't auto-scroll when paused
    if (_streamingBounds == null || _transform == null) return;

    final bounds = _streamingBounds!;
    if (bounds.xMin >= bounds.xMax) return; // Invalid bounds
    ...
    if (viewportDataPoints != null && viewportDataPoints > 0) {
      ...
      // Estimate spacing from current data range
      final pointCount = _streamingData.values.firstOrNull?.length ?? 0;
      if (pointCount > 1) {
        ...
        // Fallback: use viewportDataPoints directly as width
        viewportWidth = viewportDataPoints.toDouble();
      }
    } else {
      // EXPANDING VIEWPORT MODE: Use current transform width or full data range
      viewportWidth = _transform!.dataXRange;

      // If viewport width is too small or we're at initial state (0-1 default),
      // use full data range
      if (viewportWidth <= 1 || (_transform!.dataXMin == 0 && _transform!.dataXMax == 1)) {
        viewportWidth = bounds.xMax - bounds.xMin;
        if (viewportWidth <= 0) viewportWidth = 100; // Fallback
      }
    }
  ...
  void _animateAutoScroll() {
    ...
    final currentXMax = _transform!.dataXMax;
    // Use target width (fixed viewport) or current width (expanding viewport)
    final viewportWidth = _autoScrollTargetWidth ?? _transform!.dataXRange;
    ...
    if (delta.abs() < 0.1) {
      ...
      _updateAxesFromTransform();
      // Note: No need to regenerate elements - they update transform in paint
      markNeedsPaint();

      _autoScrollTargetXMax = null; // Clear targets
      _autoScrollTargetWidth = null;
      return;
    }

    // SMOOTH ANIMATION: Use high lerp factor for responsive but smooth motion
    // At 60fps, lerp factor of 0.15 means ~95% convergence in ~20 frames (~333ms)
    ...
    // Update axes and elements
    _updateAxesFromTransform();
    // Note: No need to regenerate elements - they update transform in _paintStreamingElements
    // Path cache remains valid as long as point count doesn't change
    markNeedsPaint();
  ...
  /// Regenerates the streaming element for a series.
  ///
  /// Called when streaming data changes or transform changes.
  void _regenerateStreamingElement(String seriesId) {
    final points = _streamingData[seriesId];

    if (points == null || points.isEmpty || _transform == null) {
      _streamingElements.remove(seriesId);
      return;
    }

    // Find matching series config from _series
    ...
    if (seriesConfig == null) {
      // No matching series config found
      return;
    }

    // PERFORMANCE: Reuse existing element if it exists (preserves path cache!)
    // Only create new element if config changed or element doesn't exist
    final existingElement = _streamingElements[seriesId];
    final streamingSeries = seriesConfig.copyWith(points: points);

    if (existingElement != null) {
      // OPTIMIZATION: Update existing element's series data instead of recreating
      // This preserves the cached path, avoiding expensive Bezier regeneration
      existingElement.updateSeries(streamingSeries);
    } else {
      // Create new element (first time or after clear)
      final element = SeriesElement(
        series: streamingSeries,
        transform: _transform!,
        seriesTheme: _theme?.seriesTheme,
        seriesIndex: 0, // Streaming is typically one series
        coordinator: coordinator,
      );
      _streamingElements[seriesId] = element;
    }
  }
  ...
  void _paintStreamingElements(Canvas canvas, Size size) {
    if (_streamingElements.isEmpty || _transform == null) return;
    ...
    // Paint each streaming element with appropriate transform
    for (final streamingElement in _streamingElements.values) {
      // Multi-axis mode: Create per-series transform with axis-specific Y bounds
      if (axisBounds != null && seriesToAxisMap != null) {
        final axisId = seriesToAxisMap[streamingElement.id];
        if (axisId != null && axisBounds.containsKey(axisId)) {
          final axisRange = axisBounds[axisId]!;
          final perSeriesTransform = _transform!.copyWith(
            dataYMin: axisRange.min,
          ...
          streamingElement.updateTransform(perSeriesTransform);
        } else {
          streamingElement.updateTransform(_transform!);
        }
      } else {
        // Single-axis mode: Use global transform
        streamingElement.updateTransform(_transform!);
      ...
      streamingElement.paint(canvas, size);
    }
  }
