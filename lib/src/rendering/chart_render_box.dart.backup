...
class ChartRenderBox extends RenderBox {
  ...
  void setStreamingData({
    ...
    if (_originalTransform != null && buffer.isNotEmpty) {
    ...
    if (_transform != null && buffer.isNotEmpty) {
      ...
      if (expandViewportWhenNotAutoScrolling) {
        ...
        final bool needsYUpdate = isInitialState || targetYMin < _transform!.dataYMin || targetYMax > _transform!.dataYMax;
        ...
        if (needsInitialXSetup || needsYUpdate) {
          _transform = _transform!.copyWith(
            ...
            dataYMin: needsYUpdate ? targetYMin : _transform!.dataYMin,
            dataYMax: needsYUpdate ? targetYMax : _transform!.dataYMax,
          );
          _updateAxesFromTransform();
        }
      }
    ...
    // PERFORMANCE: No longer using SeriesElement for streaming!
    // We now paint directly from the buffer in _paintStreamingBuffer()
    // This eliminates:
    // 1. toList() allocation every frame
    // 2. SeriesElement path cache invalidation
    // 3. Complex Bezier interpolation
  ...
  void clearStreamingData(String seriesId) {
    ...
    // This allows switching between auto-scroll and expand modes
    if (_transform != null && _originalTransform != null) {
      _transform = ChartTransform(
        ...
        dataYMin: 0,
        dataYMax: 1,
        plotWidth: _transform!.plotWidth,
        plotHeight: _transform!.plotHeight,
        invertY: _transform!.invertY,
      );
    }
  ...
  void _animateViewportExpansion() {
    ...
    final targetYMax = _expansionTargetYMax ?? _transform!.dataYMax;
    final currentYMin = _transform!.dataYMin;
    final currentYMax = _transform!.dataYMax;
  ...
  void _paintStreamingElements(Canvas canvas, Size size) {
    if (_streamingBuffers.isEmpty || _transform == null) return;

    // PERFORMANCE: Paint directly from buffer without going through SeriesElement
    // This avoids:
    // 1. List allocation from toList()
    // 2. SeriesElement path cache invalidation
    // 3. Complex interpolation recalculation
    for (final entry in _streamingBuffers.entries) {
      ...
      _paintStreamingBuffer(canvas, buffer, seriesConfig, _transform!);
    }
  }

  /// Paints streaming data directly from buffer with minimal allocation.
  ///
  /// This is a high-performance rendering path optimized for streaming:
  /// - Uses linear interpolation (O(visible) path building)
  /// - Binary search to find visible range (O(log n))
  /// - No list allocation - iterates buffer directly
  void _paintStreamingBuffer(
    ...
    // Find visible range
    final xMin = transform.dataXMin - 1; // Include 1 unit margin for edge rendering
    final xMax = transform.dataXMax + 1;

    // Binary search to find first visible point (points are sorted by X)
    int startIdx = 0;
    int endIdx = buffer.length;

    // Find first point with x >= xMin using binary search
    int lo = 0, hi = buffer.length - 1;
    while (lo < hi) {
      final mid = (lo + hi) ~/ 2;
    ...
    startIdx = lo > 0 ? lo - 1 : 0; // Include one before for edge rendering
    ...
    endIdx = (hi < buffer.length - 1) ? hi + 2 : buffer.length; // Include one after for edge rendering

    // If no visible points, skip
    if (startIdx >= endIdx) return;

    // Build path with only visible points using indexed access
    final path = Path();
    var point = buffer[startIdx];
    var plotPos = transform.dataToPlot(point.x, point.y);
