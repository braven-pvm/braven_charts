...
/// The config-form Dart emitter behind `ChartDartSourceGenerator`.
///
/// This library holds the emitter VERBATIM as it was written inside
/// `chart_dart_source_generator.dart`; the move is what lets a second
/// generator — the grammar-chain emitter in
/// `chart_grammar_source_generator.dart` — reuse the same sub-expression
/// emission (axis fields, theme, interaction, scatter encodings) instead of
/// forking a second, silently diverging copy of it.
///
/// The library is deliberately NOT exported from any barrel: the emitter is an
/// implementation detail of source generation, and the only public entry
/// points remain `ChartDartSourceGenerator` and `ChartGrammarSourceGenerator`.
/// The `emit*` members near the top of the class are the narrow, documented
/// seam the grammar emitter is allowed to call; everything else stays private.
library;
...
class ChartConfigDartEmitter {
  ...
  // ==========================================================================
  // Reuse seam for the grammar-chain emitter
  // ==========================================================================
  //
  // The grammar chain hands the SAME config objects to `.xAxis(...)`,
  // `.yAxis(...)`, `.theme(...)`, `.interaction(...)` and the scatter channel
  // templates that `BravenChartPlus` takes, so re-deriving how to write those
  // literals would mint a second emitter that drifts silently from this one.
  // These members expose the field-level writers (never the enclosing
  // `name: Type(` header, which differs between the two forms) plus the
  // warning sink, and nothing else. Everything below stays private.
  ...
  /// Writes the constructor arguments of a [YAxisConfig], without the
  /// enclosing `YAxisConfig(` / `)`. `id` is deliberately excluded: the config
  /// form carries it on the series, the grammar form on `YAxisConfig.withId`.
  void emitYAxisFields(DartSourceWriter writer, YAxisConfig axis) =>
      _emitYAxisFields(writer, axis);

  /// Writes the constructor arguments of a resolved [ChartTheme].
  void emitResolvedThemeFields(DartSourceWriter writer, ChartTheme theme) =>
      _emitResolvedThemeFields(writer, theme);

  /// Writes the constructor arguments of an [InteractionConfig].
  void emitInteractionFields(
    DartSourceWriter writer,
    InteractionConfig interaction,
  ) => _emitInteractionFields(writer, interaction);

  /// Writes `sizeEncoding: ScatterSizeEncoding(...)`, the argument name the
  /// scatter geometry verb uses too.
  void emitScatterSizeEncoding(
    DartSourceWriter writer,
    ScatterSizeEncoding encoding,
  ) => _emitScatterSizeEncoding(writer, encoding);

  /// Writes `colorEncoding: ScatterColorEncoding(...)`.
  void emitScatterColorEncoding(
    DartSourceWriter writer,
    ScatterColorEncoding encoding,
  ) => _emitScatterColorEncoding(writer, encoding);

  /// Writes `opacityEncoding: ScatterOpacityEncoding(...)`.
  void emitScatterOpacityEncoding(
    DartSourceWriter writer,
    ScatterOpacityEncoding encoding,
  ) => _emitScatterOpacityEncoding(writer, encoding);

  /// Writes `<argument>: [ScatterCategoryStyle(...), ...]`.
  void emitScatterCategoryStyles(
    DartSourceWriter writer,
    String argument,
    List<ScatterCategoryStyle> categories,
  ) => _emitScatterCategoryStyles(writer, argument, categories);

  /// Writes `<argument>: ScatterMarkerStyle(...)`.
  void emitScatterMarkerStyle(
    DartSourceWriter writer,
    String argument,
  ...
  void _emitScatterCategoryStyles(
    DartSourceWriter writer,
    String argument,
    List<ScatterCategoryStyle> categories,
  ) {
  ...
  void _emitScatterCategoryEncoding(
    ...
    writer.indented(() {
      _emitScatterCategoryStyles(writer, 'categories', encoding.categories);
  ...
  void _emitYAxis(DartSourceWriter writer, String argument, YAxisConfig axis) {
    ...
    writer.indented(() => _emitYAxisFields(writer, axis));
    writer.writeLine('),');
  }

  void _emitYAxisFields(DartSourceWriter writer, YAxisConfig axis) {
  ...
  void _emitThemeReference(DartSourceWriter writer) {
    ...
    final expression = _themeReferenceExpression;
    if (expression != null) {
  ...
  void _emitResolvedTheme(DartSourceWriter writer, ChartTheme theme) {
    ...
    writer.indented(() => _emitResolvedThemeFields(writer, theme));
    writer.writeLine('),');
  }

  void _emitResolvedThemeFields(DartSourceWriter writer, ChartTheme theme) {
    writer.namedArgument(
      'backgroundColor',
  ...
  void _emitInteraction(
    ...
    writer.indented(() => _emitInteractionFields(writer, interaction));
    writer.writeLine('),');
  }

  void _emitInteractionFields(
    DartSourceWriter writer,
    InteractionConfig interaction,
  ) {
    _valueIf(writer, 'enabled', interaction.enabled, defaultValue: true);
    _valueIf(writer, 'enableZoom', interaction.enableZoom, defaultValue: true);
