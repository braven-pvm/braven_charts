...
/// | a concentric composition whose rings DIVERGE in `donutStyle`, `selectionStyle`, `unit`, `sliceRadiusConfig` or `sliceGroupingConfig`, or in which ANY ring carries a series `color` | blocked — `DonutMark` holds ONE of each for the whole composition and `_lowerDonutRings` stamps it onto every ring, and unlike the single-donut `_lowerDonut` beside it never passes `mark.color` at all. Sharing a NON-DEFAULT value is fine; divergence is not, and no ring may carry a series colour even when every ring carries the same one. These reach the author through the catch-all row below rather than a reason of their own — a gap, since an author can hit them from ordinary config-form Dart |
