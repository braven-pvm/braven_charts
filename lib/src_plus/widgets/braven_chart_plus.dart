...
import '../axis/axis_color_resolver.dart';
...
class _BravenChartPlusState extends State<BravenChartPlus> {
  ...
  // Auto-detection state for multi-axis normalization
  // Computed during _rebuildElements when normalizationMode == auto
  DetectionResult? _autoDetectionResult;
  List<YAxisConfig>? _autoGeneratedYAxes;

  /// Returns the effective Y-axis configurations with resolved colors.
  ///
  /// Priority:
  /// 1. User-provided [widget.yAxes] if not empty (with colors resolved from series)
  /// 2. Auto-generated axes when normalizationMode is auto and detection triggered
  /// 3. null if single-axis mode
  ///
  /// Axis colors are resolved as follows:
  /// - Explicit [YAxisConfig.color] takes priority
  /// - Otherwise, color is derived from the first bound series
  /// - Neutral grey if no series bound
  List<YAxisConfig>? get effectiveYAxes {
    if (widget.yAxes != null && widget.yAxes!.isNotEmpty) {
      // Resolve colors for user-provided axes
      return _resolveAxisColors(widget.yAxes!);
    }
    return _autoGeneratedYAxes;
  }

  /// Resolves colors for axes that don't have explicit colors.
  ///
  /// For each axis without an explicit color, derives the color from
  /// the first series bound to that axis.
  List<YAxisConfig> _resolveAxisColors(List<YAxisConfig> axes) {
    final effectiveSeries = widget.series.whereType<ChartSeries>().toList();
    final colorMap = AxisColorResolver.resolveAllAxisColors(
      axes: axes,
      series: effectiveSeries,
    );

    // Return axes with resolved colors applied
    return axes.map((axis) {
      if (axis.hasExplicitColor) {
        return axis; // Keep explicit color
      }
      final resolvedColor = colorMap[axis.id];
      if (resolvedColor != null && resolvedColor != AxisColorResolver.neutralColor) {
        return axis.copyWith(color: resolvedColor);
      }
      return axis; // Keep as-is (will use neutral grey in renderer)
    }).toList();
  }

  /// Whether multi-axis mode is currently active (either explicit or auto-detected).
  bool get isEffectiveMultiAxisMode {
    final axes = effectiveYAxes;
    return axes != null && axes.isNotEmpty;
  }
  ...
  void _rebuildElements() {
    ...
    // ==================== AUTO-DETECTION FOR MULTI-AXIS ====================
    // Perform auto-detection when normalizationMode is NormalizationMode.auto
    // This runs after effectiveSeries is computed but before bounds calculation
    if (widget.normalizationMode == NormalizationMode.auto && effectiveSeries.length >= 2) {
      _autoDetectionResult = NormalizationDetector.detect(effectiveSeries);

      if (_autoDetectionResult!.shouldNormalize) {
        // Auto-generate Y-axis configurations if not already provided
        if (widget.yAxes == null || widget.yAxes!.isEmpty) {
          final suggestions = NormalizationDetector.suggestAxes(effectiveSeries);
          _autoGeneratedYAxes = _generateYAxesFromSuggestions(suggestions, effectiveSeries);
        }
      } else {
        // Detection says no normalization needed - clear auto-generated axes
        _autoGeneratedYAxes = null;
      }
    } else if (widget.normalizationMode != NormalizationMode.auto) {
      // Not in auto mode - clear auto-detection state
      _autoDetectionResult = null;
      _autoGeneratedYAxes = null;
    }
  ...
  /// Generates YAxisConfig list from auto-detected suggestions.
  ///
  /// Called when normalizationMode is auto and detection indicates
  /// normalization is needed but no yAxes were provided.
  List<YAxisConfig> _generateYAxesFromSuggestions(
    List<SuggestedAxisConfig> suggestions,
    List<ChartSeries> effectiveSeries,
  ) {
    final yAxes = <YAxisConfig>[];

    // Position assignment for auto-generated axes
    final positions = [
      YAxisPosition.left,
      YAxisPosition.right,
    ...
    for (var i = 0; i < suggestions.length && i < 4; i++) {
      ...
      // Get color from first series in this axis group
      Color? axisColor;
      for (final seriesId in suggestion.seriesIds) {
        final series = effectiveSeries.firstWhere(
          (s) => s.id == seriesId,
          orElse: () => effectiveSeries.first,
        );
        axisColor = series.color;
        break;
      }

      // Get unit from first series if available
      String? unit;
      for (final seriesId in suggestion.seriesIds) {
        final series = effectiveSeries.firstWhere(
          (s) => s.id == seriesId,
