...
void main() {
  ...
  group('BravenChartPlus Widget API Surface', () {
    ...
    test('theme - exposed via useDarkTheme in schema', () {
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('useDarkTheme'), isTrue, reason: 'ChartTheme exposed as useDarkTheme boolean');
    });
    ...
    test('MISSING: theme - full ChartTheme customization NOT exposed', () {
      ...
      expect(true, isTrue, reason: 'DOCUMENTED GAP: Full theme customization not exposed');
    });
    ...
    test('MISSING: legendStyle - full LegendStyle customization NOT exposed', () {
      ...
      // - spacing ❌
      // - textStyle ❌
      expect(true, isTrue, reason: 'DOCUMENTED GAP: Only legendPosition exposed, not full LegendStyle');
    });
    ...
    test('MISSING: grid - full GridConfig customization NOT exposed', () {
      ...
      // - majorWidth ❌
      // - minorWidth ❌
      expect(true, isTrue, reason: 'DOCUMENTED GAP: Only on/off grid exposed, not full GridConfig');
    });
    ...
    test('showYScrollbar - NOT separately exposed', () {
      ...
      expect(true, isTrue, reason: 'DOCUMENTED: showScrollbar controls both X and Y');
    });
    ...
    test('MISSING: scrollbarTheme - NOT exposed', () {
      ...
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('scrollbarTheme'), isFalse, reason: 'DOCUMENTED GAP: scrollbarTheme not exposed');
    });
    ...
    test('MISSING: interactionConfig - full InteractionConfig NOT exposed', () {
      ...
      expect(true, isTrue, reason: 'DOCUMENTED GAP: Only basic interaction flags exposed');
    });
    ...
    test('annotationController - NOT exposed (runtime only)', () {
      ...
      expect(true, isTrue, reason: 'EXPECTED: annotationController is runtime API, not declarative');
    });

    test('interactiveAnnotations - NOT exposed', () {
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('interactiveAnnotations'), isFalse, reason: 'DOCUMENTED GAP: interactiveAnnotations flag not exposed');
    });
    ...
    test('streamingController - NOT exposed (runtime API)', () {
      expect(true, isTrue, reason: 'EXPECTED: streamingController is runtime API');
    });

    test('liveStreamController - NOT exposed (runtime API)', () {
      expect(true, isTrue, reason: 'EXPECTED: liveStreamController is runtime API');
    });
    ...
    test('showDebugInfo - NOT exposed', () {
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('showDebugInfo'), isFalse, reason: 'EXPECTED: debug flag not needed for LLM');
    });

    test('showToolbar - NOT exposed', () {
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('showToolbar'), isFalse, reason: 'DOCUMENTED GAP: showToolbar not exposed');
    });

    test('loadingWidget - NOT exposed (custom widget)', () {
      expect(true, isTrue, reason: 'EXPECTED: custom widgets cannot be configured via JSON');
    });
    ...
    test('autoScrollConfig - NOT exposed', () {
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('autoScrollConfig'), isFalse, reason: 'DOCUMENTED GAP: autoScrollConfig not exposed');
    });

    test('streamingConfig - NOT exposed', () {
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      expect(props.containsKey('streamingConfig'), isFalse, reason: 'DOCUMENTED GAP: streamingConfig not exposed');
    });
  });
  ...
  group('LineChartSeries API Surface', () {
    test('id - exposed via schema series.id', () {
      final tool = agent_tools.CreateChartTool();
      final seriesItems = (tool.inputSchema['properties']['series']['items']) as Map;
    ...
    test('yAxis nested object exists (replaces flat yAxisId)', () {
      ...
      expect(props.containsKey('yAxis'), isTrue, reason: 'yAxis nested object should exist');
    });
    ...
    test('MISSING: isXOrdered - NOT exposed', () {
      ...
      final seriesItems = (tool.inputSchema['properties']['series']['items']) as Map;
      final props = seriesItems['properties'] as Map;
      expect(props.containsKey('isXOrdered'), isFalse, reason: 'DOCUMENTED GAP: isXOrdered optimization hint not exposed');
    });
    ...
    test('MISSING: metadata - NOT exposed', () {
      ...
      final seriesItems = (tool.inputSchema['properties']['series']['items']) as Map;
      final props = seriesItems['properties'] as Map;
      expect(props.containsKey('metadata'), isFalse, reason: 'DOCUMENTED GAP: metadata not exposed');
    });
    ...
    test('MISSING: annotations (per-series) - NOT exposed', () {
      ...
      final seriesItems = (tool.inputSchema['properties']['series']['items']) as Map;
      final props = seriesItems['properties'] as Map;
      expect(props.containsKey('annotations'), isFalse, reason: 'DOCUMENTED GAP: per-series annotations not exposed, use chart-level');
    });
  });
  ...
  group('BarChartSeries API Surface', () {
    ...
    test('MISSING: borderRadius - NOT exposed', () {
      ...
      final seriesItems = (tool.inputSchema['properties']['series']['items']) as Map;
      final props = seriesItems['properties'] as Map;
      expect(props.containsKey('barBorderRadius'), isFalse, reason: 'Need to check if braven_charts BarChartSeries has borderRadius');
    });
  });
  ...
  group('XAxisConfig API Surface', () {
    ...
    test('MISSING: showCrosshairLabel - NOT exposed', () {
      expect(xAxisProps.containsKey('showCrosshairLabel'), isFalse, reason: 'DOCUMENTED GAP: showCrosshairLabel not exposed');
    });

    test('MISSING: crosshairLabelPosition - NOT exposed', () {
      expect(xAxisProps.containsKey('crosshairLabelPosition'), isFalse, reason: 'DOCUMENTED GAP: crosshairLabelPosition not exposed');
    });

    test('MISSING: labelStyle - NOT exposed', () {
      expect(xAxisProps.containsKey('labelStyle'), isFalse, reason: 'DOCUMENTED GAP: labelStyle (TextStyle) not exposed');
    });

    test('MISSING: axisColor - NOT exposed', () {
      expect(xAxisProps.containsKey('axisColor'), isFalse, reason: 'DOCUMENTED GAP: axisColor not exposed');
    });
  });
  ...
  group('YAxisConfig API Surface (via series yAxis nested object)', () {
    ...
    test('MISSING: visible - NOT exposed', () {
      expect(yAxisProps.containsKey('visible'), isFalse, reason: 'DOCUMENTED GAP: visible not exposed in yAxis');
    });

    test('MISSING: tickCount - NOT exposed', () {
      expect(yAxisProps.containsKey('tickCount'), isFalse, reason: 'DOCUMENTED GAP: tickCount not exposed in yAxis');
    });
  });
  ...
  group('Enum Completeness - braven_charts vs braven_agent', () {
    test('YAxisPosition - all values mapped', () {
      ...
      expect(agentValues, equals(chartsValues), reason: 'AxisPosition must match YAxisPosition exactly');
    });
    ...
    test('LineInterpolation - all values mapped', () {
      ...
      expect(agentValues, equals(chartsValues), reason: 'Interpolation must match LineInterpolation exactly');
    });

    test('NormalizationMode - all values mapped', () {
      final chartsValues = NormalizationMode.values.map((e) => e.name).toSet();
      final agentValues = agent.NormalizationModeConfig.values.map((e) => e.name).toSet();
      expect(agentValues, equals(chartsValues), reason: 'NormalizationModeConfig must match NormalizationMode exactly');
    });
    ...
    test('LegendPosition - all braven_charts values reachable', () {
      ...
      expect(chartsValues, contains('topCenter'), reason: 'mapped from agent "top"');
      expect(chartsValues, contains('bottomCenter'), reason: 'mapped from agent "bottom"');
      expect(chartsValues, contains('centerLeft'), reason: 'mapped from agent "left"');
      expect(chartsValues, contains('centerRight'), reason: 'mapped from agent "right"');
      ...
      expect(chartsValues, contains('center'), reason: 'DOCUMENTED GAP: center position not exposed');
    });

    test('AnnotationAnchor - all values mapped', () {
      final chartsValues = AnnotationAnchor.values.map((e) => e.name).toSet();
      final agentValues = agent.AnnotationPosition.values.map((e) => e.name).toSet();
      expect(agentValues, equals(chartsValues), reason: 'AnnotationPosition must match AnnotationAnchor exactly');
    });
    ...
    test('MarkerShape - braven_agent MarkerStyle is subset', () {
      ...
      for (final value in agentValues) {
        expect(chartsValues, contains(value), reason: 'MarkerStyle.$value must exist in MarkerShape');
      }
      ...
      if (missing.isNotEmpty) {
        ...
        expect(missing, containsAll(['star', 'cross', 'plus']), reason: 'DOCUMENTED GAP: MarkerShape has more values than MarkerStyle exposes');
      }
    });
    ...
    test('CrosshairMode - used by renderer', () {
      ...
      expect(chartsValues, containsAll(['vertical', 'horizontal', 'both', 'none']));
    });

    test('CrosshairDisplayMode - used by renderer', () {
      final chartsValues = CrosshairDisplayMode.values.map((e) => e.name).toSet();
  ...
  group('Annotation Types API Surface', () {
    ...
    test('MISSING: PointAnnotation - NOT exposed', () {
      ...
      final annotProps = annotationsItems['properties'] as Map;
      final typeEnum = annotProps['type']['enum'] as List;
      expect(typeEnum, isNot(contains('pointAnnotation')), reason: 'DOCUMENTED GAP: PointAnnotation not exposed, use marker instead');
    });

    test('TrendAnnotation - exposed via trendLine type', () {
      // braven_charts has TrendAnnotation for trend lines - NOW EXPOSED
      final tool = agent_tools.CreateChartTool();
      final props = tool.inputSchema['properties'] as Map;
      final annotationsItems = (props['annotations']['items']) as Map;
      final annotProps = annotationsItems['properties'] as Map;
      final typeEnum = annotProps['type']['enum'] as List;
      expect(typeEnum, contains('trendLine'), reason: 'TrendAnnotation exposed as trendLine annotation type');
    });
  });
  ...
  group('API Coverage Summary', () {
    test('COUNT: BravenChartPlus widget-level gaps', () {
      ...
      // 9. streamingConfig
      const widgetGaps = 9;
      expect(widgetGaps, equals(9), reason: '9 widget-level feature gaps documented');
    });
    ...
    test('COUNT: Series-level gaps', () {
      ...
      const seriesGaps = 7;
      expect(seriesGaps, equals(7), reason: '7 series-level feature gaps documented');
    });
    ...
    test('COUNT: Axis config gaps', () {
      ...
      const axisGaps = 8;
      expect(axisGaps, equals(8), reason: '8 axis-level feature gaps documented');
    });

    test('COUNT: Annotation gaps', () {
      // 1. PointAnnotation - still not exposed (use marker instead)
      // TrendAnnotation - NOW EXPOSED as trendLine type
      const annotationGaps = 1;
      expect(annotationGaps, equals(1), reason: '1 annotation type gap documented');
    });
    ...
    test('TOTAL: All documented gaps', () {
      ...
      // Updated: TrendAnnotation now exposed, reducing annotation gaps from 2 to 1
      const totalGaps = 9 + 7 + 8 + 1 + 2;
      expect(totalGaps, equals(27), reason: '27 TOTAL documented feature gaps between braven_charts and braven_agent');
    });
  });
}
